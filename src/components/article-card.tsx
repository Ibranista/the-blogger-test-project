...
export function ArticleCard({ article }: ArticleCardProps) {
  ...
  return (
    <article className="group relative">
      <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-accent/10 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10" />
      ...
      <Card className="relative overflow-hidden border-0 bg-card/50 backdrop-blur-sm rounded-2xl shadow-sm hover:shadow-xl transition-all duration-500 group-hover:scale-[1.02] group-hover:bg-card/80">
        <Link href={`/post/${article.id}`} className="block">
          <div className="aspect-[4/3] relative overflow-hidden rounded-t-2xl">
            ...
            <div className="absolute top-4 left-4 z-10">
              <Badge className="bg-background/90 text-foreground backdrop-blur-sm border-border/50 hover:bg-background transition-colors">
        ...
        <CardHeader className="pb-4 pt-6 px-6 relative z-20">
          <Link href={`/post/${article.id}`} className="block">
            <h3 className="text-xl font-heading font-bold text-foreground group-hover:text-primary transition-colors duration-300 text-balance line-clamp-2 leading-tight">
              {article.acf.title}
            </h3>
          </Link>
        </CardHeader>
        ...
        <div className="absolute inset-0 border-2 border-primary/0 group-hover:border-primary/20 rounded-2xl transition-all duration-500 -z-10" />
      </Card>
    </article>
  );
}
